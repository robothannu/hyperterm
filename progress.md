...
## Current Task
- **completed** — AGENTS.md goal 추출 fallback chain에 `## Project Intent` 추가 (1 line patch). 사용자 ocr_app_ios 카드가 "No overview data"로 떨어지던 문제 수정.
- (이전) Codex 지원 (3 sprints, all PASS), main 머지 + dist 빌드 (HyperTerm-0.1.0-arm64.dmg 22:11). divider 누적 carryover fix 1건 처리.
...
### 사용자 보고
- ocr_app_ios 카드가 `git unavailable` + `No overview data — add CLAUDE.md or progress.md`로 표시.

### 진단
- ocr_app_ios에 `.git` 없음 → `git unavailable`은 정상 표시 (dashboard.ts:575-577).
- `AGENTS.md`는 존재하나 첫 섹션이 `## Project Intent`. 기존 fallback chain은 `## Overview` / `## Project Overview` / `## Project Background` / `extractFirstParagraphAfterH1`만 본다. H1 직후 헤더이므로 H1 단락 추출도 null → goal 없음 → progress/handoff 부재로 "No overview data" 폴백 메시지.

### 수정
- `src/main/workspace-reader.ts:507-509`: `## Project Intent` fallback 한 줄 추가 (Project Background 다음).
- `npx tsc --noEmit` 0 errors.

### 미커밋 상태
- `src/main/workspace-reader.ts` 5 lines added (+5/-0). 사용자 검증 후 커밋 예정.
...
- 참고: 오늘(05-06) AGENTS.md fallback 1줄 패치는 harness 스킵 — 1줄 surgical fix.
